...
This script supports both ASFE and RBFE payloads and writes:
1) submission.yaml metadata.
2) zenodo_description.md (Zenodo-style description document).
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate submission.yaml and zenodo_description.md from an "
            "AlchemicalArchive or AlchemicalNetwork JSON file (.json or .json.bz2)."
        )
    )
    parser.add_argument(
        "input_file",
        type=Path,
        help="Path to the AlchemicalArchive or AlchemicalNetwork file (.json or .json.bz2).",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("."),
        help="Directory to write outputs (default: current working directory).",
    )
    parser.add_argument(
        "--submission-id",
        type=str,
        default=None,
        help="Optional submission id for submission.yaml (defaults to date + network slug).",
    )
    parser.add_argument(
        "--keywords",
        type=str,
        default="openfe,alchemicalarchive",
        help="Comma-separated keywords for submission.yaml.",
    )
    parser.add_argument(
        "--author",
        action="append",
        default=[],
        help="Author entry for submission.yaml (repeat for multiple authors).",
    )
    parser.add_argument(
        "--archive-doi",
        type=str,
        default="",
        help="Long-term DOI for this submission (if available).",
    )
    parser.add_argument(
        "--archive-provider",
        type=str,
        default="",
        help="Archive provider (e.g., Zenodo).",
    )
    parser.add_argument(
        "--license",
        type=str,
        default="CC-BY-4.0",
        help="License for submission metadata.",
    )
...
def main() -> None:
    ...
    submission_id = args.submission_id or _default_submission_id(network_key)
    ...
    submission_yaml_text = _make_submission_yaml(
        ...
        archive_doi=args.archive_doi,
        archive_provider=args.archive_provider,
